...
namespace HotelProject.WebUI.Controllers
{
    public class ServiceController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("http://localhost:5123/api/Service");// Veri listelemek istediğimiz için "GetAsync" metodunu kullandık. Bu metot bizden bir url ister yani nereye istekte bulunacağımızı belirtiriz
        ...
        public async Task<IActionResult> AddService(CreateServiceDto createServiceDto)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:5123/api/Service", stringContent);// Ekleme yapabilmek için "PostAsync" metodunu kullanırız
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:5123/api/Service/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
