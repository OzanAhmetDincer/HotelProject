...
namespace HotelProject.WebUI.Controllers
{
    public class StaffController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("http://localhost:12091/api/Staff");// Veri listelemek istediğimiz için "GetAsync" metodunu kullandık. Bu metot bizden bir url ister yani nereye istekte bulunacağımızı belirtiriz
        ...
        public async Task<IActionResult> AddStaff(AddStaffViewModel model)  
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:12091/api/Staff", stringContent);// Ekleme yapabilmek için "PostAsync" metodunu kullanırız
        ...
        public async Task<IActionResult> DeleteStaff(int id)
        {
            ...
            var responseMessage = await client.DeleteAsync($"http://localhost:12091/api/Staff/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:12091/api/Staff/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> UpdateStaff(UpdateStaffViewModel model)
        {
            ...
            var responseMessage = await client.PutAsync("http://localhost:12091/api/Staff/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
